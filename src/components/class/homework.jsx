...
const ClassHomeworkComponent = () => {
  ...
  const fetchHomeWorkDet = async () => {
    const resp = await api.post("/tac/homeworkStudent/getHomeWorkDet", {
      homeworkId: dataId,
      classId,
    });
    setData(resp.data);
  ...
  const fetchHomeworkHistory = async () => {
    const resp = await api.post("/tac/homeworkStudent/getHistoryVesion", {
      homeworkId: dataId,
      classId,
      groupId: null,
  ...
  const submitStuHomework = async () => {
    const resp = await api.post("/tac/homeworkStudent/submitHomeWork", {
      homeworkId: dataId,
      groupId: null,
        ...
        <Col lg={12} xs={24}>
          <Card title={"作业内容"}>
            <Space direction={"vertical"} style={{ display: "flex" }}>
              ...
              {data.homeworkDocListResList &&
              data.homeworkDocListResList.length > 0 ? (
                <Card>
                  <List
                ...
                >
                  <div>
                    <Space align={"center"} direction={"horizontal"}>
                      ...
                      <span>
                        {data.overFlag && data.activeFlag === 1
                          ? "补交"
                          : "交作业"}
                      </span>
                      {data.correctStatu !== 0 ? (
                        <span style={{ fontSize: "13px" }}>
                          {formatter.format(
                            new Date(Number.parseInt(data.submitTime ?? "0"))
                          )}
                        </span>
                      ) : null}
                    </Space>
            ...
            <Space direction={"vertical"} style={{ display: "flex" }}>
              ...
              {data.correctStatu === 0 ? (
                <>
                  <Upload
                    customRequest={(e) => {
                      const obs = new OBSUploader();
                      obs.upload({
                        file: e.file,
                        onProgress: e.onProgress,
                        onSuccess: async (res) => {
                          const resp = await saveDoc({
                            docName: e.file.name,
                          ...
                          delete resp.data.id;
                          setStuHomeworkFile([...stuHomeworkFile, resp.data]);
                          e.onSuccess();
                        },
                        onError: e.onError,
                      });
          ...
          <List
            ...
            renderItem={(item) => (
              <List.Item>
                <Card
                  style={{ width: "100%" }}
                  title={`${item.name} ${formatter.format(new Date(Number.parseInt(item.submitTime)))}`}
                >
